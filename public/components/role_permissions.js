Vue.component('role-permissions-component', {
    ...
    template: `
        <div class="card">
        <div class="card-body">
            <h3>Choose permissions for <span class="text-blue font-weight-bold">  {{ role.name }}   <i :class="role.icon" class="float-right"></i></span></h3>
        </div>
        <div class="card-body">
            <div class="row form-group ">
                <div class="form-check col-md-6" :key="index" v-for="(permission,index) in permissions">
                    <input class="form-check-input" type="checkbox" :id="permission.name" style="cursor: pointer"
