Vue.component('profile-linkedin-chat', {
                        ...
                        <div class="users-container p-2 border">
                            ...
                            <ul class="users border" style="max-height: 685px;overflow-y: scroll">
                                ...
                                >
                                    <div class="user">
                                        <img
                                            :src="getParticipant(thread.data.member)[0].picture??'/dist/img/avatar3.png'">
                                    </div>
                                    <p class="name-time">
                                        <span class="name">{{ getParticipant(thread.data.member)[0].firstName }}
                                            {{ getParticipant(thread.data.member)[0].lastName }}
                                        </span>
                                        <br/>
                                ...
                                <div class="card-body">
                                    <div class="direct-chat-messages" id="messages"
                                        ...
                                        <div class="direct-chat-msg" v-for="(message,key) in messages" :key="key">
                                            ...
                                            <img class="direct-chat-img" :src="messageHelper(message).userAvatar" style="margin-top: 8px;">
                                            <div class="direct-chat-text"
                                                ...
                                                <p style="margin-bottom: 0!important;line-height: 1.42857">
                                                    <span
                                                        style="font-weight: 600;margin-right: 10px">{{ messageHelper(message).userFullName }}</span>
                                                    ...
                                                    <span
                                                        class="direct-chat-timestamp"><small>{{ messageHelper(message).date_diff }}</small></span>
                                                </p>
                                                <p> {{ messageHelper(message).text }}</p>
                                                <p v-if="messageHelper(message).media">
                                                    <img :src="messageHelper(message).media.previewgif.url">
                                                </p>
                                                <p v-if="messageHelper(message).attachments">
                                                    <a :href="messageHelper(message).attachments[0].reference" v-if="messageHelper(message).attachments[0].mediaType.includes('application')"> {{messageHelper(message).attachments[0].name}}</a>
                                                    <img v-else :src="messageHelper(message).attachments[0].reference">
                                                </p>
                                            </div>
                                        </div>
    ...
    methods: {
        ...
        sortThreads: function (key = '') {
            ...
            if (key.length) {
                a = this.threads.filter(function (item) {
                    ...
                    if (item.data.member[1].firstName.toLowerCase().includes(key.toLowerCase()) || item.data.member[1].lastName.toLowerCase().includes(key.toLowerCase())) {
                        con = true;
                    }
                    return con;
                });
        ...
        messageHelper: function (message) {
            ...
            if (message.user_entityUrn === this.user.linkedin_urn_id) {
                newMessage.userFullName = this.user.first_name + ' ' + this.user.last_name;
                newMessage.userAvatar = this.user.avatar ? '/storage/' + this.user.avatar : '/dist/img/avatar5.png';
            } else {
                newMessage.userFullName = this.selectedConversation.fullName;
                newMessage.userAvatar = this.selectedConversation.avatar;
            }
        ...
        getConversation: function (thread = {}, syncData = []) {
            ...
            if (Object.keys(thread).length) {
                let participant = this.getParticipant(thread.data.member)[0];
            ...
            window.location.hash =   this.selectedConversation.id;


            this.$http.get(`/dashboard/linkedin/conversations/${this.selectedConversation.id}/user/${this.user.id}/messages`)
                .then((response) => {
                    this.message = ''
